...
public class droidger extends PApplet {
...
float changeVel = 0.85f;               // modifies all velocities
...
/// Aura
float circleFactor = 1.25f;             // size of aura per obstacle size
...
public void showScore() {
  ...
  if(startTimer + runDuration <= millis()) {
    ...
    pg.text(" || Your trial version timer has run out. Starting score is updated...          ||" , pgWidth*2/4, pgHeight*3.5f/5);
    pg.text(" || The next run you can do will be lengthened by "+ PApplet.parseInt(newStart) + "sec, so enjoy...     ||" , pgWidth*2/4, pgHeight*3.5f/5 +50);
    pg.text(" || " + PApplet.parseInt(score) +" * "+ nf(scoreRate, 0, 3) + "/3 +" + PApplet.parseInt(highScore) + " * " + nf(scoreRate, 0, 3) + "/4 ||" , pgWidth*2/4, pgHeight*3.5f/5 +100);
    pg.text(" || new starting score: " + PApplet.parseInt(newStart) + " press a key to reset||" , pgWidth*2/4, pgHeight*3.5f/5 +150);
  }
...
public void showMenu() {
  ...
  // } else {
    ...
    pg.text("vol key to", pgWidth*2/4, pgHeight*3.45f/4 -50);
...
class Enemy {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "droidger" };
    if (passedArgs != null) {
